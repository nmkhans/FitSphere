  ...
  return (
    <section className="py-16 px-6 md:px-12 space-y-8 relative">
      ...
      <div className="flex justify-between flex-col-reverse md:flex-row gap-6 items-center">
        ...
        <div className=" bottom-8 z-50 liquid-jiggle bg-[#B6FB69] right-8 p-4 rounded-full  fixed">
          <CartButton />
        </div>
      </div>
      ...
      <div className="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {products.map((product) => (
          <div
            key={product._id}
            className="group relative bg-card text-card-foreground min-h-[360px] rounded-xl shadow-lg overflow-hidden flex flex-col"
          >
            {/* Product Image with transition */}
            <div className="relative w-full h-70 transition-all duration-200 group-hover:h-56">
              <Image
                src={product.photo}
                alt={product.name}
                fill
            ...
            {/* Hover Buttons */}
            <div
              className="absolute bottom-0 w-full flex gap-2 px-4 pb-4 translate-y-4 
               transition-all opacity-0 group-hover:opacity-100 duration-300  group-hover:translate-y-0"
            >
              <Link
                href={`/store/${product._id}`}
                className="flex-1 bg-primary text-primary-foreground hover:bg-primary/90 text-center py-1 rounded-md font-semibold transition"
              >
                View Details
              </Link>
